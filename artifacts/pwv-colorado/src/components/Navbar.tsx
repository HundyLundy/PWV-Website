...
import { Menu, X, ChevronDown, Home, FileText, Zap, BarChart2 } from "lucide-react";
...
export function Navbar({ onScrollTo }: { onScrollTo: (id: string) => void }) {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 flex flex-col">
      {/* Rep contact bar */}
      <div className="py-1.5 px-4 text-xs font-medium text-center text-white flex justify-center items-center gap-2 flex-wrap"
        style={{ background: '#0374A7' }}>
        <span>{REP_INFO.name} — Colorado Rep</span>
        ...
        <div className="max-w-7xl mx-auto h-20 flex items-center justify-between">
          ...
          >
            ...
            <div className="leading-tight hidden sm:block" style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 600 }}>
              ...
              <span className="text-lg" style={{ color: '#DEC600' }}>Valve — Colorado</span>
            </div>
          </button>
          ...
          <div className="hidden md:flex items-center gap-5">
            <a href={`tel:${REP_INFO.phone.replace(/-/g, '')}`} className="text-sm font-semibold text-gray-300 hover:text-white transition-colors">
              {REP_INFO.phone}
            </a>
            <button
        ...
        {mobileOpen && (
          ...
          >
            <div className="p-4 flex flex-col gap-1">
              ...
              <a href={`tel:${REP_INFO.phone.replace(/-/g, '')}`} className="text-sm text-white/40 text-center mt-2">
                {REP_INFO.phone}
              </a>
            </div>
          </motion.div>
