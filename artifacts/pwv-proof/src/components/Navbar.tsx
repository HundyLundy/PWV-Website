...
import { Menu, X, ChevronDown, Home, Zap, BarChart2, MapPin, Phone, BookOpen, Building2 } from "lucide-react";
    ...
    >
        ...
        <nav className="hidden md:flex items-center gap-6">
          {SECTION_LINKS.map((l) => (
            <button key={l.label} onClick={() => scrollTo(l.href)}
              className="text-sm font-medium transition-colors hover:opacity-80" style={{ color: '#2E4A5A' }}>
              {l.label}
            </button>
          ))}
          ...
          <div className="relative" ref={exploreRef}>
            ...
            {exploreOpen && (
              <div className="absolute right-0 top-full mt-3 w-72 rounded-2xl shadow-xl overflow-hidden"
          ...
          <a href="/" className="px-5 py-2 rounded-full text-white text-sm font-semibold transition-all hover:-translate-y-0.5"
            style={{ background: '#0374A7', boxShadow: '0 4px 14px rgba(3,116,167,0.3)' }}>
            Request Assessment
          </a>
        </nav>

        <button className="md:hidden p-2 transition-colors" style={{ color: '#0A1F3A' }} onClick={() => setMenuOpen(!menuOpen)}>
      ...
      {menuOpen && (
        <div className="md:hidden border-t px-4 py-6 flex flex-col gap-1"
            ...
            {EXPLORE_LINKS.map((link) => {
              ...
              return (
                <a key={link.href} href={link.href} onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-3 py-2.5 transition-colors hover:opacity-70" style={{ color: '#2E4A5A' }}>
          ...
          <a href="/" className="mt-3 px-5 py-3 rounded-full text-white text-sm font-semibold text-center"
            style={{ background: '#0374A7' }} onClick={() => setMenuOpen(false)}>
            Request Assessment
          </a>
