...
import {
  ORANGE_APARTMENT,
  ORANGE_APARTMENT_LABELS,
  ORANGE_APARTMENT_CONSUMPTION,
} from "@/data/orangeApartmentVerified";

// ─── DATA (sourced from orangeApartmentVerified.ts — DO NOT hardcode here) ──

const CONSUMPTION = ORANGE_APARTMENT_CONSUMPTION as unknown as { period: string; gpd: number }[];

const ANNUAL_CONTEXT = [
  { label: "Before Smart Valve™", cost: Math.round(ORANGE_APARTMENT.beforeGalPerDay * 365 / 748 * ORANGE_APARTMENT.utilityRatePerCCF) },
  { label: "After Smart Valve™",  cost: Math.round(ORANGE_APARTMENT.afterGalPerDay  * 365 / 748 * ORANGE_APARTMENT.utilityRatePerCCF) },
];
...
export default function OrangeApartmentNJ() {
  return (
    <div className="min-h-screen bg-white text-slate-900">
      <Helmet>
        <title>500 Main St, Orange NJ: 26% Water Reduction, $3,566/yr Saved | Smart Valve™ Case Study</title>
        <meta name="description" content="Flow Dynamics case study: Orange Apartment Building, 500 Main Street, Orange NJ — 16-unit apartment achieved 26% metered water reduction, $3,566/yr savings, 8-month payback." />
        <link rel="canonical" href="https://www.perfectwatervalve.com/results/orange-apartment-nj" />
        <meta property="og:title" content="500 Main St, Orange NJ: 26% Water Reduction, $3,566/yr" />
        <meta property="og:description" content="16-unit apartment, Orange NJ — 26% reduction, 371,000 gal/yr saved, $3,566 annual savings, 8-month payback." />
        <meta property="og:url" content="https://www.perfectwatervalve.com/results/orange-apartment-nj" />
      </Helmet>
      ...
      <section className="pt-32 pb-14 px-4 sm:px-6 lg:px-8" style={{ background: "#E8EFF7" }}>
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-wrap items-center gap-2 mb-5">
            ...
            <span className="text-xs uppercase tracking-widest" style={{ color: "#4A7085" }}>Multifamily Residential · Orange, NJ · Verified by Managing Agent</span>
          </div>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black mb-4 leading-tight" style={{ color: "#0A1F3A" }}>
            Orange Apartment Building — 500 Main St, NJ<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-sky-500">26% Water Reduction · $3,566/yr Saved</span>
          </h1>
          <p className="text-lg max-w-3xl mb-10" style={{ color: "#2E4A5A" }}>
            16-unit apartment building. Municipal water supply, 1″ meter, no central AC.
            Verified by managing agent — occupancy constant with minimal turnover.
          </p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <StatPill value={ORANGE_APARTMENT_LABELS.reduction}     label="Water Reduction"    sub="Metered, M&V verified" />
            <StatPill value={ORANGE_APARTMENT_LABELS.gallonsSaved}  label="Gallons Saved/Year" sub="371K gal annually" />
            <StatPill value={ORANGE_APARTMENT_LABELS.annualSavings} label="Annual Savings"      sub="Water only" />
            <StatPill value={ORANGE_APARTMENT_LABELS.payback}       label="Payback Period"      sub="One of the fastest on record" />
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto space-y-10">
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <ChartCard
              ...
              caption={`Before: Feb–Nov 2013 · After: Nov 2013–Sep 2014 · Rate: ${ORANGE_APARTMENT_LABELS.utilityRate}`}
              note={`Consumption dropped from ${ORANGE_APARTMENT_LABELS.beforeGPD} to ${ORANGE_APARTMENT_LABELS.afterGPD} gallons per day — a ${ORANGE_APARTMENT_LABELS.reduction} reduction. Occupancy constant with minimal turnover. No central AC.`}
            >
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={CONSUMPTION} margin={{ top: 10, right: 24, left: 10, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 11, fill: "#4A7085" }} domain={[2000, 4500]} tickFormatter={(v) => v.toLocaleString()} />
            ...
            <ChartCard
              ...
              caption={`Projected at ${ORANGE_APARTMENT_LABELS.utilityRate} · USD`}
              note={`At ${ORANGE_APARTMENT_LABELS.utilityRate}, a ${ORANGE_APARTMENT_LABELS.reduction} reduction on 3,860 gal/day saves approximately ${ORANGE_APARTMENT_LABELS.annualSavings} per year — with an ${ORANGE_APARTMENT_LABELS.payback} payback.`}
            >
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={ANNUAL_CONTEXT} margin={{ top: 10, right: 24, left: 20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                  <XAxis dataKey="label" tick={{ fontSize: 11, fill: "#4A7085" }} />
                  <YAxis tick={{ fontSize: 11, fill: "#4A7085" }} tickFormatter={(v) => `$${(v / 1000).toFixed(1)}K`} domain={[8000, 15000]} />
        ...
        <div className="max-w-4xl mx-auto">
          <div className="grid sm:grid-cols-3 gap-6">
            <div className="rounded-2xl p-6 bg-white border border-slate-200 text-center">
              <div className="text-4xl font-black mb-2" style={{ color: "#0374A7" }}>{ORANGE_APARTMENT_LABELS.reduction}</div>
            ...
            <div className="rounded-2xl p-6 bg-white border border-slate-200 text-center">
              <div className="text-4xl font-black mb-2" style={{ color: "#059669" }}>{ORANGE_APARTMENT_LABELS.annualSavings}</div>
              <div className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: "#0A1F3A" }}>Annual Savings</div>
              <div className="text-xs" style={{ color: "#4A7085" }}>{ORANGE_APARTMENT_LABELS.gallonsSaved} gallons/yr</div>
            </div>
            <div className="rounded-2xl p-6 text-center" style={{ background: "#0374A7" }}>
              <div className="text-4xl font-black mb-2 text-white">{ORANGE_APARTMENT_LABELS.payback}</div>
              <div className="text-xs font-bold uppercase tracking-wider mb-1 text-white">Payback Period</div>
              <div className="text-xs" style={{ color: "rgba(255,255,255,0.75)" }}>Among the fastest in this portfolio</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-14 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              { title: "26% reduction — among the highest in the NJ multifamily portfolio", body: "At 26%, this is one of the strongest reductions recorded in any New Jersey apartment case study — consistent with similar small-building, 1\" meter configurations where meter over-spin has the highest proportional impact." },
              { title: "8-month payback on a 1\" meter installation", body: `At ${ORANGE_APARTMENT_LABELS.utilityRate} combined water rate, the ${ORANGE_APARTMENT_LABELS.annualSavings} annual savings generated a full return on investment in just ${ORANGE_APARTMENT_LABELS.payback} — well inside a single fiscal year.` },
              { title: "No central AC — seasonally stable baseline", body: "The absence of central air conditioning eliminates summertime HVAC-related water consumption variation, making the 26% reduction a direct measure of the Smart Valve™ effect on base consumption." },
              { title: "Occupancy constant — managing agent verified", body: "Occupancy remained constant with minimal turnover throughout both measurement periods. Data sourced directly from the managing agent — not self-reported by ownership." },
            ].map((f) => (
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8" style={{ background: "#F8FAFC" }}>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-4 text-sm">
            {[
              { label: "Location",       value: `${ORANGE_APARTMENT.address}, ${ORANGE_APARTMENT.city}` },
              { label: "Property Type",  value: ORANGE_APARTMENT.propertyType },
              { label: "Meter Size",     value: ORANGE_APARTMENT.meterSize },
              { label: "Water Source",   value: ORANGE_APARTMENT.waterSource },
              { label: "Utility Rate",   value: `${ORANGE_APARTMENT_LABELS.utilityRate} (water only)` },
              { label: "Data Source",    value: ORANGE_APARTMENT.source },
            ].map((d) => (
